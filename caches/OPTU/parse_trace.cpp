...
uint64_t parseTraceFile(std::vector<trEntry> & opt_trace,
    ...
    for (const auto & trace : traces) {
        ...
        if(lastSeen.count(idSize)>0) {
            ...
            opt_trace[lastSeen[idSize]].nextSeen = reqc;
            ...
            opt_trace[lastSeen[idSize]].utility = 1.0L/utilityDenominator;
            assert(opt_trace[lastSeen[idSize]].utility>0);
        } else {
            uniqc++;
        }
        opt_trace.emplace_back(id,size);
